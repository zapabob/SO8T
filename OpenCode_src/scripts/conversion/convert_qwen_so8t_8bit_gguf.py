...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('logs/qwen_so8t_gguf_conversion.log'),
...
class QwenSO8T8bitGGUFConverter:
    ...
    def __init__(self, config_path: str, output_dir: str = "models/qwen_so8t_8bit_gguf"):
    ...
    def load_so8t_model(self) -> SO8TTransformerForCausalLM:
        ...
        logger.info(f"デバイス: {device} (GGUF化はCPUで実行)")
        ...
        logger.info(f"   - SO8T回転次元: {model_config.rotation_dim}")
        
        # モデル初期化
        model = SO8TTransformerForCausalLM(model_config)
        
        # 8bit量子化設定（CPU実行のため簡易版）
        logger.info("2/3: 8bit量子化設定中...")
        logger.info("CPU実行のため、重みの型変換のみ実行")
        
        # モデルをfloat32に統一（CPU実行用）
        model = model.float()
        logger.info("モデルをfloat32に変換完了")
    ...
    def convert_to_gguf_format(self, model: SO8TTransformerForCausalLM, tokenizer: AutoTokenizer) -> Dict:
        ...
        gguf_config = {
            ...
            "so8t_multihead_attention": self.config['so8t']['multihead_attention'],
            ...
            "converter": "QwenSO8T8bitGGUFConverter"
        }
    ...
    def save_gguf_model(self, gguf_data: Dict) -> Path:
        ...
        torch.save(gguf_data, gguf_path)
        
        # ファイルサイズ計算
        file_size_gb = gguf_path.stat().st_size / (1024**3)
        logger.info(f"統合GGUFファイル保存: {gguf_path}")
    ...
    def create_model_card(self, gguf_path: Path) -> None:
        ...
        model_card = f"""# Qwen2.5-7B-Instruct SO8T Transformer 8bit GGUF Model

## Model Information
- **Model Type**: Qwen2.5-7B-Instruct SO8T Transformer (8bit quantized)
...
## Citation
```bibtex
@misc{{qwen_so8t_transformer_8bit,
  title={{Qwen2.5-7B-Instruct SO8T Transformer: 8bit Quantized SO(8) Group Structure Transformer}},
  author={{SO8T Team}},
  year={{2025}},
...
"""
    ...
    def convert(self) -> Path:
        ...
        try:
            logger.info("=" * 80)
            logger.info("Qwen2.5-7B-Instruct SO8T Transformer 8bit量子化GGUF変換開始")
...
def main():
    ...
    parser.add_argument("--config", type=str, default="configs/qwen_so8t_8bit_gguf_config.yaml",
                       help="設定ファイルパス")
    parser.add_argument("--output_dir", type=str, default="models/qwen_so8t_8bit_gguf",
                       help="出力ディレクトリ")
    
    args = parser.parse_args()
    
    # 変換器作成
    converter = QwenSO8T8bitGGUFConverter(args.config, args.output_dir)
    
    # 変換実行
    gguf_path = converter.convert()
    
    print(f"\n[SUCCESS] Qwen2.5-7B-Instruct SO8T Transformer 8bit GGUF変換完了！")
