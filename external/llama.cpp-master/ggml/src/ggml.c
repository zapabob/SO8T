...
static const char * GGML_OP_SYMBOL[GGML_OP_COUNT] = {
    ...
    "so8_rotation(x,R)",
    "so8_non_commutative(x,R_safe,R_cmd)",
};
...
// SO8T specific operations

struct ggml_tensor * ggml_so8_rotation(
        struct ggml_context * ctx,
        struct ggml_tensor  * input,
        struct ggml_tensor  * rotation_matrix) {
    GGML_ASSERT(input != NULL);
    GGML_ASSERT(rotation_matrix != NULL);
    GGML_ASSERT(ggml_is_matrix(rotation_matrix));
    GGML_ASSERT(rotation_matrix->ne[0] == 8);
    GGML_ASSERT(rotation_matrix->ne[1] == 8);
    
    // Ensure input has 8 dimensions in the last axis
    GGML_ASSERT(input->ne[0] == 8 || (ggml_n_dims(input) >= 2 && input->ne[0] == 8));
    
    struct ggml_tensor * result = ggml_new_tensor(ctx, input->type, ggml_n_dims(input), input->ne);
    
    result->op = GGML_OP_SO8_ROTATION;
    result->src[0] = input;
    result->src[1] = rotation_matrix;
    
    return result;
}

struct ggml_tensor * ggml_so8_non_commutative(
        struct ggml_context * ctx,
        struct ggml_tensor  * input,
        struct ggml_tensor  * rotation_safe,
        struct ggml_tensor  * rotation_cmd) {
    GGML_ASSERT(input != NULL);
    GGML_ASSERT(rotation_safe != NULL);
    GGML_ASSERT(rotation_cmd != NULL);
    GGML_ASSERT(ggml_is_matrix(rotation_safe));
    GGML_ASSERT(ggml_is_matrix(rotation_cmd));
    GGML_ASSERT(rotation_safe->ne[0] == 8 && rotation_safe->ne[1] == 8);
    GGML_ASSERT(rotation_cmd->ne[0] == 8 && rotation_cmd->ne[1] == 8);
    ...
    result->op = GGML_OP_SO8_NON_COMMUTATIVE;
    result->src[0] = input;
    result->src[1] = rotation_safe;
    result->src[2] = rotation_cmd;
