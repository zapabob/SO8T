...
extern "C" {
    ...
    enum ggml_op {
        ...
        GGML_OP_SO8_ROTATION,        // 8x8 orthogonal rotation matrix multiplication
        GGML_OP_SO8_NON_COMMUTATIVE, // Non-commutative gate (R_safe -> R_cmd)
    ...
    // SO8T specific operations
    // Apply 8x8 orthogonal rotation matrix to input tensor
    // Input: [batch_size, seq_len, 8] or [batch_size, 8]
    // Rotation: [8, 8] orthogonal matrix
    // Output: [batch_size, seq_len, 8] or [batch_size, 8]
    GGML_API struct ggml_tensor * ggml_so8_rotation(
            struct ggml_context * ctx,
            struct ggml_tensor  * input,
            struct ggml_tensor  * rotation_matrix);

    // Apply non-commutative gate: R_safe -> intermediate -> R_cmd
    // This implements the SO8T safety mechanism where R_safe and R_cmd
    // are applied in sequence with intermediate state preservation
    GGML_API struct ggml_tensor * ggml_so8_non_commutative(
            struct ggml_context * ctx,
            struct ggml_tensor  * input,
