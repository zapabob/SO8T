...
# NKATユーティリティ
from src.utils.nkat_utils import (
    NKATRewardFunction,
    NKATThermostat,
...
# SO8Tモデル
from src.models.so8t_thinking_model import create_so8t_thinking_model
...
def train_so8t_ppo(config: SO8TPPOConfig):
    ...
    # 再開時は累計時間を維持
    if resume_step > 0:
        last_checkpoint_time = time.time()  # 再開時は現在時刻から開始
    ...
    global_step = resume_step  # 再開時はresume_stepから開始
    ...
    final_session_info = {
        'global_step': global_step,
        'total_training_time': total_training_time,
        'final_checkpoint_time': time.time(),
