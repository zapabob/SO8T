...
def hf_cli_available() -> bool:
    try:
        subprocess.run(["py", "-3", "-m", "huggingface_hub.commands.huggingface_cli", "--version"], check=True, capture_output=True)
        return True
    except Exception:
...
def download_dataset(dataset_id: str, out_dir: Path) -> Dict[str, Any]:
    ...
    # Use module call instead of entry point to avoid launcher errors
    cmd = ["py", "-3", "-m", "huggingface_hub.commands.huggingface_cli", "download", dataset_id, "--repo-type", "dataset", "--local-dir", str(out_dir)]
...
def main() -> int:
    ...
    for item in tqdm(datasets, desc="Downloading HF Datasets"):
        ds_id = item["id"]
