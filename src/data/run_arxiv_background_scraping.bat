...
chcp 65001 >nul
echo ================================================================================
echo SO8T統制によるArxiv・オープンアクセス論文完全自動バックグラウンドスクレイピング
echo ================================================================================
echo.
echo [INFO] SO8Tモデルの四重推論を使って、完全自動でバックグラウンド実行されます
echo [INFO] Arxivの全ジャンルとオープンアクセス論文をスクレイピングします
...
echo [INFO] Arxivの全ジャンル（cs, math, physics, q-bio, q-fin, stat, eess, econ）をスクレイピングします
echo [INFO] オープンアクセス論文サイト（PubMed Central, DOAJ, PLOS ONE, BioRxiv, medRxiv, HAL, CORE）もスクレイピングします
echo [INFO] バックグラウンドで実行され、自動再起動機能が有効です
echo.

REM バックグラウンドで実行
start /B "" "%PYTHON%" -3 scripts\data\so8t_arxiv_background_scraping.py --output "%OUTPUT_DIR%" --daemon --auto-restart --max-restarts 10 --restart-delay 3600.0

if %errorlevel% equ 0 (
    echo [OK] バックグラウンドスクレイピングを開始しました
    echo [INFO] ログファイル: %LOG_DIR%\so8t_arxiv_background_scraping.log
) else (
...
echo [INFO] ログを確認するには: type %LOG_DIR%\so8t_arxiv_background_scraping.log
echo.
