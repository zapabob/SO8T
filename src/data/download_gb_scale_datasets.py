...
"""
            ...
            if result.returncode == 0:
                ...
                logger.info(f"[OK] SlimPajama downloaded: {size_gb:.2f} GB")
                return True
            else:
                logger.error(f"[NG] SlimPajama download failed: {result.stderr}")
                return False

        except Exception as e:
            logger.error(f"[NG] SlimPajama download error: {e}")
            return False
...
"""
            ...
            if result.returncode == 0:
                size_gb = self._get_dir_size_gb(dataset_dir)
                self.downloaded_size += size_gb
                logger.info(f"[OK] StarCoder sample downloaded: {size_gb:.2f} GB")
                return True
            else:
                logger.error(f"[NG] StarCoder download failed: {result.stderr}")
                return False

        except Exception as e:
            logger.error(f"[NG] StarCoder download error: {e}")
            return False
...
"""
            ...
            if result.returncode == 0:
                size_gb = self._get_dir_size_gb(dataset_dir)
                self.downloaded_size += size_gb
                logger.info(f"[OK] FineWeb-Edu sample downloaded: {size_gb:.2f} GB")
                return True
            else:
                logger.error(f"[NG] FineWeb-Edu download failed: {result.stderr}")
                return False

        except Exception as e:
            logger.error(f"[NG] FineWeb-Edu download error: {e}")
            return False
    ...
    def run_download_pipeline(self, datasets: List[str]) -> bool:
        ...
        if self.downloaded_size >= 10.0:  # At least 10GB
            logger.info("[OK] Sufficient data acquired for initial SO8T training")
            return True
        else:
            logger.warning("[WARN]  Data volume still insufficient - consider manual downloads")
            return False
