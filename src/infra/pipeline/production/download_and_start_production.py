...
# 大規模データダウンローダー
try:
    from src.data.download_large_datasets import LargeDatasetDownloader
...
# 本番環境起動スクリプト
try:
    from src.pipelines.start_production_pipeline import main as start_production_main
...
def download_datasets(datasets: list = None, max_samples: int = None):
    ...
    try:
        from src.data.download_large_datasets import DEFAULT_DATASETS
