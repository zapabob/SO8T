...
class IntegratedMoonshotPipeline2025_2026:
    ...
    def execute_full_pipeline(self, use_existing_datasets: bool = True) -> None:
        ...
        if os.getenv("SO8T_DYNAMIC_DISPATCH", "0") == "1":
            dispatcher = self.project_root / "tools" / "dynamic_skill_dispatcher.py"
            if dispatcher.exists():
                try:
                    subprocess.run(
                        ["py", "-3", str(dispatcher), "execute-workflow", "--workflow", "config/subagent_tasks.yaml"],
                        check=True,
                        cwd=self.project_root,
                    )
                except subprocess.CalledProcessError as exc:
                    logger.warning("Dynamic dispatch schedule failed: %s", exc)
        ...
        if start_idx <= 1:
            routing = self._route_phase(
                "enrich",
                "Run multi-domain data enrichment (academic, pop-culture, world affairs, pharma)",
                tags=["enrichment", "multi_domain", "pharma"],
            )
            enriched_paths = self.run_multi_domain_enrichment()
            for enriched_path in enriched_paths:
                if enriched_path not in dataset_paths:
                    dataset_paths.append(enriched_path)
            self._save_checkpoint(
                "enrich",
                {
        ...
        if start_idx <= 2:
            routing = self._route_phase(
                "reward",
                "Apply quadrality reward strategy for GRPO",
                tags=["reward", "grpo", "policy"],
            )
            reward_path = self.apply_reward_strategy()
            if reward_path and reward_path not in dataset_paths:
                dataset_paths.append(reward_path)
            self._save_checkpoint(
                "reward",
                {
