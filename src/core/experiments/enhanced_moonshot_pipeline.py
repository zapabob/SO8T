...
class EnhancedMoonshotPipeline:
    ...
    def _maybe_run_unsloth(self, phase: str) -> bool:
        ...
        if os.getenv("SO8T_DRYRUN") == "1":
            ...
            return False
        script = Path("src/training/train_unsloth_so8t.py")
        if not script.exists():
            # Fallback for older versions
            script = Path("scripts/training/train_unsloth_so8t.py")
            if not script.exists():
                logger.warning("Unsloth training script not found at src/ or scripts/")
                return False
        ...
        if os.getenv("SO8T_TRAINING_CONFIG"):
            ...
            # デフォルトで borea_training.json を使用
            default_config = Path("src/infrastructure/config/borea_training.json")
