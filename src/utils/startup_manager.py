...
class StartupManager:
    ...
    def register(self, extra_args: list[str] = None):
        ...
        try:
            ...
            args_str = " ".join(extra_args) if extra_args else "--use-existing-datasets"
            # .bat ファイルの内容を作成
            # py -3 を使用し、スクリプトがあるディレクトリに cd してから実行
            project_root = self.script_path.parent
            bat_content = f"""@echo off
cd /d "{project_root}"
start /B py -3 "{self.script_path.name}" {args_str} > moonshot_pipeline_startup.log 2>&1
timeout /t 5 > nul
REM monitor_pipeline.py exists in src/utils/ now
py -3 src/utils/monitor_pipeline.py
pause
"""
