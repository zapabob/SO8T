...
# 既存モジュールのインポート
try:
    from src.data.massive_parallel_crawler import ParallelWebCrawler, MASSIVE_CRAWL_CONFIG
except ImportError:
    # フォールバック: 直接パス指定
...
try:
    from src.data.specialized_crawlers import (
...
try:
    from src.data.robots_compliance import create_compliance_checker
except ImportError:
    import importlib.util
    spec = importlib.util.spec_from_file_location(
        "robots_compliance",
...
try:
    from src.data.pipeline_cleanse_and_sample import DataPipeline
except ImportError:
    import importlib.util
    spec = importlib.util.spec_from_file_location(
...
# 用途別データセット生成のインポート
try:
    from so8t_mmllm.src.data.purpose_specific_dataset_generator import PurposeSpecificDatasetGenerator
...
from src.data.auto_labeler_thinking import ThinkingAutoLabeler

# MLOps可視化モジュールのインポート
try:
    from src.utils.mlops_visualizer import MLOpsVisualizer
...
# NSFW分類器のインポート
try:
    from src.data.train_nsfw_classifier import NSFWClassifier
...
# マルチモーダルNSFW検知のインポート
try:
    from src.data.multimodal_nsfw_detector import MultimodalNSFWDetector
...
try:
    from so8t_mmllm.src.data.multimodal_web_crawler import MultimodalWebCrawler
