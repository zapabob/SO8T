...
## Git 追跡対象の整理 (2026-02-06)

- **内容**: 全ての `.txt` および `.log` ファイルを Git の追跡対象から除外（`requirements.txt` を除く）。
- **実施手順**:
  1. `.gitignore` の末尾に追跡除外ルールを追加。
  2. `git rm --cached` により、既にインデックスに含まれていた当該ファイルを追跡解除。
  3. 変更をコミットおよびリモートへプッシュ。
- **結果**: ログファイルや一時的なテキストファイルがリポジトリに含まれなくなり、リポジトリの軽量化とクリーンな状態を維持。
