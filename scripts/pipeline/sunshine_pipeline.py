...
def run_sunshine_experiment(run_type: str = "baseline") -> Dict[str, Any]:
    ...
    try:
        ...
        if config.so8_config:
            ...
            model = attach_nkat_adapters(model, target_layers=config.so8_config['target_layers'])

            # SO(8)アダプターに高い学習率を設定
            adapter_lr = 1e-3  # LoRAの10倍
            for name, param in model.named_parameters():
                if 'nkat_adapter' in name:
                    param.requires_grad_(True)
                    # パラメータグループで学習率を設定
                    print(f"Setting high LR ({adapter_lr}) for {name}")
        else:
        ...
        # パラメータグループ設定（SO8Tの場合、アダプターに高い学習率）
        if config.so8_config:
            # LoRAパラメータとアダプターパラメータを分離
            lora_params = []
            adapter_params = []
            other_params = []
            ...
            # パラメータグループ
            param_groups = [
                {'params': lora_params, 'lr': config.training_config['learning_rate']},
                {'params': adapter_params, 'lr': config.training_config['learning_rate'] * 10},  # 10倍
                {'params': other_params, 'lr': config.training_config['learning_rate']}
            ]
            print(f"Parameter groups: LoRA({len(lora_params)}), Adapter({len(adapter_params)}), Other({len(other_params)})")
        else:
            param_groups = None
        ...
        if param_groups:
            ...
            trainer.optimizer = torch.optim.AdamW(param_groups, lr=config.training_config['learning_rate'])
