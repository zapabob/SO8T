"""
NSFW/危険コンテンツ分類器の訓練スクリプト

scikit-learnベースの分類器を訓練し、弱教師ラベリングを実装。
"""
...
LABEL_CATEGORIES = [
    "safe",
    "nsfw_soft",  # 合法かつ同意ベースの成人向け表現
    "nsfw_block",  # 暴力・違法・児童・差別等（ブロック用）
    "violence",
    "harassment",
    "self_harm",
    "weapons_detail",  # 具体的手順（拒否用）
...
class NSFWClassifier:
    ...
    def train(
        ...
        print(f"[INFO] Label distribution: {dict(zip(*np.unique(labels, return_counts=True)))}")
        ...
        print("\nConfusion Matrix:")
        print(confusion_matrix(y_test, y_pred, labels=LABEL_CATEGORIES))
...
def main():
    parser = argparse.ArgumentParser(description="Train NSFW/危険コンテンツ分類器")
    parser.add_argument(
        "--mode",
        type=str,
        choices=["train", "label"],
    ...
    if args.mode == "train":
        ...
        classifier.train(args.labeled_data)
