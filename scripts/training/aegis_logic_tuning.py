...
def create_synthetic_logic_data():
    ...
    try:
        ...
        # データを増強
        expanded_examples = synthetic_examples * 20  # 60サンプル作成（元の3倍）

        train_dataset = Dataset.from_list(expanded_examples[:-6])  # 54サンプル
        test_dataset = Dataset.from_list(expanded_examples[-6:])   # 6サンプル
        ...
        # datasetsライブラリが利用できない場合
        print("[WARNING] datasets library not available, using simple list format")

        expanded_examples = synthetic_examples * 20
        return {
            "train": expanded_examples[:-6],
            "test": expanded_examples[-6:]
        }
...
def setup_logic_tuning_config():
    ...
    config = {
        "learning_rate": 2e-5,  # 学習率を上げる
        "num_epochs": 1,        # エポック数を1に減らす
        "batch_size": 1,        # バッチサイズを1に
        "gradient_accumulation_steps": 4,  # 勾配蓄積を減らす
        "lora_rank": 4,         # LoRAランクを小さく
        "target_modules": ["qkv_proj"],  # qkv_projのみに特化
        "lora_alpha": 8,        # alphaも小さく
        "lora_dropout": 0.1,    # dropoutを増やす
        "warmup_steps": 5,      # warmupを減らす
        "logging_steps": 5,     # ログ頻度を上げる
        "save_steps": 25,       # 保存頻度を上げる
        "evaluation_strategy": "steps",
        "eval_steps": 25,       # 評価頻度を上げる
        "max_length": 256,      # シーケンス長を短く
...
def train_logic_layer(base_model_path: str, output_dir: str = "models/aegis_logic_tuned"):
    ...
    # データセット形式をチェックして統一的に扱う
    if hasattr(dataset["train"], 'map'):
        # Datasetオブジェクトの場合
        tokenized_dataset = dataset.map(
            tokenize_fn,
            batched=True,
            num_proc=1,  # num_procを1に減らす（メモリ節約）
            remove_columns=dataset["train"].column_names
        )
    else:
        # リスト形式の場合、Datasetに変換してから処理
        from datasets import Dataset
        ...
        tokenized_dataset = temp_dataset.map(
            ...
            batched=True,
            num_proc=1,
            remove_columns=temp_dataset["train"].column_names
        )
    ...
    training_args = TrainingArguments(
        ...
        per_device_train_batch_size=min(train_config["batch_size"], 2),  # バッチサイズを制限
        per_device_eval_batch_size=min(train_config["batch_size"], 2),
    ...
    # データセットがリスト形式の場合、Datasetに変換
    if not hasattr(train_dataset, 'map'):
        from datasets import Dataset
